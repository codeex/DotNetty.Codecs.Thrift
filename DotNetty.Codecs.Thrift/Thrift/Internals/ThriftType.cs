    ...
    internal sealed class ThriftType
    {
        ...
        private ThriftType(Type type)
        {
            ...
            if (TypeInfo.IsEnum)
            {
                ...
                {
                    throw new Exception("ThriftParsingException.NonInt32Enum(TypeInfo)");
                }
            ...
            {
                throw new Exception("ThriftParsingException.UnknownValueType( TypeInfo )");
            }
            ...
            var mapInterfaceAndArgs = GetInstantiableVersion(TypeInfo, typeof(IDictionary<,>), typeof(Dictionary<,>), p => new Exception("ThriftParsingException.UnsupportedMap( p ) "));
            ...
            var setInterfaceAndArgs = GetInstantiableVersion(TypeInfo, typeof(ISet<>), typeof(HashSet<>), p => new Exception("ThriftParsingException.UnsupportedSet( p ) "));
            if (setInterfaceAndArgs != null)
            {
                if (mapInterfaceAndArgs != null)
                {
                    throw new Exception("ThriftParsingException.CollectionWithOrthogonalInterfaces( TypeInfo )");
                }
            ...
            var listInterfaceAndArgs = GetInstantiableVersion(TypeInfo, typeof(IList<>), typeof(List<>), p => new Exception("ThriftParsingException.UnsupportedList( p ) "));
        ...
        private static Tuple<TypeInfo, Type[]> GetInstantiableVersion(TypeInfo typeInfo, Type interfaceType, Type concreteType, Func<TypeInfo, Exception> errorProvider)
        {
            ...
            foreach (var iface in typeInfo.ImplementedInterfaces.Where(i => i.GenericTypeArguments.Length > 0))
            {
                ...
                if (unboundIface == interfaceType)
                {
                    ...
                    {
                        throw new Exception("ThriftParsingException.CollectionWithMultipleGenericImplementations( typeInfo )");
                    }
